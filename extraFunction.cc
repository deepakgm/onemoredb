...
void MyFucntion ::FireUpExistingDatabase()
{
...
void MyFucntion ::traverse(Operator *root ,int outputSet){
    ...
    switch (root->getType()) {
        case SELECT_FILE:
            if (outputSet == 2) ((SelectFileOperator*)root)->print();
            else ((SelectFileOperator*)root)->run();
            break;
        case SELECT_PIPE:
            traverse(((SelectPipeOperator*)root)->left, outputSet);
            if (outputSet == 2) ((SelectPipeOperator*)root)->print();
            else ((SelectPipeOperator*)root)->run();
            break;
        case PROJECT:
            traverse(((ProjectOperator*)root)->left, outputSet);
            if (outputSet == 2) ((ProjectOperator*)root)->print();
            else ((ProjectOperator*)root)->run();
            break;
        case GROUPBY:
            traverse(((GroupByOperator*)root)->left, outputSet);
            if (outputSet == 2) ((GroupByOperator*)root)->print();
            else ((GroupByOperator*)root)->run();
            break;
        case SUM:
            traverse(((SumOperator*)root)->left, outputSet);
            if (outputSet == 2) ((SumOperator*)root)->print();
            else ((SumOperator*)root)->run();
            break;
        case DUPLICATE_REMOVAL:
            traverse(((DuplicateRemovalOperator*)root)->left, outputSet);
            if (outputSet == 2) ((DuplicateRemovalOperator*)root)->print();
            else ((DuplicateRemovalOperator*)root)->run();
            break;
        case JOIN:
            traverse(((JoinOperator*)root)->left, outputSet);
            traverse(((JoinOperator*)root)->right, outputSet);
            if (outputSet == 2) ((JoinOperator*)root)->print();
            else ((JoinOperator*)root)->run();
            break;
        default:
            cerr << "ERROR: Unspecified node!" << endl;
            exit(-1);
    }
    if (outputSet == 2) cout << endl << "*******************************************************" << endl;
}